// MixedFractionTests+MultipliableTests.swift
// RationalsTests
//
...
extension MixedFractionTests {
	...
	func test_multiplicationReturnsNonLike() {
		// Given
		let multiplicand: TestSubject = .init(1, 1, on: 2)
		let multiplier: TestSubject = .init(2, on: 2)

		// When
		let product: TestSubject = multiplicand * multiplier

		// Then
		XCTAssertEqual(product.mixed(), .init(1, 2, on: 4))
	}
